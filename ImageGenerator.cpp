...
void ImageGenerator::setDownloadPath(const QString &path) { m_downloadPath = path; }

QString ImageGenerator::downloadPath() const { return m_downloadPath; }

void ImageGenerator::setOutputFileName(const QString &fileName) { m_outputFileName = fileName; }

QString ImageGenerator::outputFileName() const { return m_outputFileName; }

void ImageGenerator::generateImage(const QString &prompt) {
    if (m_apiKey.isEmpty()) {
...
void ImageGenerator::handleImageGenerationReply(QNetworkReply *reply) {
    qDebug() << Q_FUNC_INFO;
